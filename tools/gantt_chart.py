...
def get_tasks_from_user()
    ...
    for i in range(num_tasks)
        task_id = input(fEnter task ID {i+1} )
        start_date = input(fEnter start date for task {i+1} (YYYY-MM-DD) )
        end_date = input(fEnter end date for task {i+1} (YYYY-MM-DD) )
        task_name = input(fEnter task name for task {i+1} )
